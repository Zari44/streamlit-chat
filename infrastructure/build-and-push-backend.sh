#!/bin/bash
# Script to build and push the backend Docker image to the remote instance
...
echo -e "${GREEN}Building and pushing backend Docker image to instance: $INSTANCE_IP${NC}"
...
ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no ec2-user@$INSTANCE_IP << EOF
    ...
    rm -f /tmp/goatbot-images/${BACKEND_IMAGE}.tar
    echo "Backend image loaded successfully!"
EOF

echo ""
echo -e "${GREEN}✓ Backend Docker image built and pushed successfully!${NC}"
echo ""
